#!/usr/bin/env python3
"""
Run NetG-PSO ablation without network (zero degrees, no groups).
Appends results to experiments/experiments_results_full.csv
"""
...
def main(args):
    ...
    # prepare net_meta no-network
    net_meta = {'graph': None, 'degrees': {n:0.0 for n in feature_names}, 'partition': {n:-1 for n in feature_names}}
    for run_i in range(args.runs):
        ...
        pso = NetGPSO(Xtr, ytr, feature_names, net_meta, pop_size=args.pop_size, max_iter=args.max_iter, alpha=args.alpha, seed=seed)
        ...
        row = {
            ...
            'test_f1':test_f1,
            'selected_file':f"experiments/selected_netg_nonet_seed{seed}_a{args.alpha:.2f}.txt"
        }
        ...
        print(f"Done ablation seed={seed} test_acc={test_acc:.4f} sel={mask.sum()} time={runtime:.1f}s")
