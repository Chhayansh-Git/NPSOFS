#!/usr/bin/env python3
"""
Run DE-FS baseline for multiple seeds and append results to experiments/experiments_results_full.csv
"""
import argparse, time, os, pickle
import numpy as np
import pandas as pd
from sklearn.model_selection import train_test_split
from src.pso.de_fs import DEFS
import numpy as np
...
def main(args):
    ...
    for run_i in range(args.runs):
        ...
        de = DEFS(Xtr, ytr, feature_names, pop_size=args.pop_size, max_iter=args.max_iter, alpha=args.alpha, seed=seed)
        best_vec, best_score = de.run(verbose=False)
        # decode using DE's sigmoid mapping
        mask = (de.decode(best_vec)).astype(int)
        # evaluate holdout
        ...
        row = {
            ...
            'test_f1':test_f1,
            'selected_file':f"experiments/selected_de_seed{seed}_a{args.alpha:.2f}.txt"
        }
        ...
        print(f"Done DE-FS seed={seed} test_acc={test_acc:.4f} sel={mask.sum()} time={runtime:.1f}s")
    print("DE-FS runs complete.")
